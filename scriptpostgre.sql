-- Role: augusto
-- DROP ROLE IF EXISTS augusto;

CREATE ROLE augusto WITH
  LOGIN
  SUPERUSER
  INHERIT
  CREATEDB
  CREATEROLE
  NOREPLICATION
  ENCRYPTED PASSWORD 'SCRAM-SHA-256$4096:LvNl2UdqXrLs3RHkf/L/Fw==$aS9eq9jlX9/A8wBF/FxJGWwthwUo8tpyYHi8CilbhTs=:AbF8uTjY1wRNONd9KbjIm+Nbc2qlyWKs3sLDYqWriMo=';

ALTER ROLE augusto SET search_path TO elmasri, "$user", public;
...
OMMENT ON TABLE elmasri.trabalha_em IS 'tabela que armazena quais funcionarios trabalham em quais projetos';
...
CREATE TABLE elmasri.dependente (
              ...
              CONSTRAINT dependentes_pk PRIMARY KEY (nome_dependente, cpf_funcionario)
);
...
ALTER TABLE elmasri.dependente ADD CONSTRAINT sexo_m_f_dependente CHECK( sexo IN ('M', 'F'));

ALTER TABLE elmasri.funcionario ADD CONSTRAINT Sexo_m_f_funcionario CHECK ( sexo IN ('M', 'F'));

ALTER TABLE elmasri.funcionario ADD CONSTRAINT salario_funcionario CHECK (salario BETWEEN '1000' AND '200000' );

ALTER TABLE elmasri.trabalha_em ADD CONSTRAINT horas_trabalha_em CHECK (horas BETWEEN '0' AND '200');
